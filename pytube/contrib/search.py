...
import logging
# from .....TierListBot.TierListBot import debug
# Local imports
from .. import YouTube
...
class Search:
    ...
    def fetch_and_parse(self, continuation=None):
        ...
        # debug(raw_results, 'raw_results.json')
        ...
        if item_renderer:
            ...
            for video_details in raw_video_list:
                ...
                # print(f'shelfRenderer{"shelfRenderer" in video_details}')
                if 'shelfRenderer' in video_details:
                    continue
                
                # print(f'radioRenderer{"radioRenderer" in video_details}')
                # Skip auto-generated "mix" playlist results
                if 'radioRenderer' in video_details:
                    continue
                
                # print(f'playlistRenderer{"playlistRenderer" in video_details}')
                # Skip playlist results
                if 'playlistRenderer' in video_details:
                    continue

                # print(f'channelRenderer{"channelRenderer" in video_details}')
                # Skip channel results
                if 'channelRenderer' in video_details:
                    continue

                # print(f'horizontalCardListRenderer{"horizontalCardListRenderer" in video_details}')
                # Skip 'people also searched for' results
                if 'horizontalCardListRenderer' in video_details:
                    continue

                # print(f'didYouMeanRenderer{"didYouMeanRenderer" in video_details}')
                # Can't seem to reproduce, probably related to typo fix suggestions
                if 'didYouMeanRenderer' in video_details:
                    continue

                # print(f'backgroundPromoRenderer{"backgroundPromoRenderer" in video_details}')
                ...
                # print(f'videoRenderer{"videoRenderer" in video_details}')
                ...
                vid_thumbnail = vid_renderer['thumbnail'],
                
                if 'viewCountText' in vid_renderer:
                    if 'runs' in vid_renderer['viewCountText']:
                        vid_view_count_text = vid_renderer['viewCountText']['runs'][0]['text'] + " viewers"
                    else:
                    ...
                    if stripped_text == 'No':
                        ...
                        # print(vid_view_count)
                else:
                    vid_view_count = 0
                if 'lengthText' in vid_renderer:
                    vid_length = vid_renderer['lengthText']['simpleText']
                    vid_length_access = vid_renderer['lengthText']['accessibility']['accessibilityData']['label']
                    components = vid_length.split(':')
                    ...
                    vid_length_seconds = hours * 3600 + minutes * 60 + seconds
                else:
                    vid_length = None
                    vid_length_access = "Live Video"
                    vid_length_seconds = None
                ...
                vid_metadata = {
                    ...
                    'length_seconds': vid_length_seconds,
                    'length': vid_length_access,
                    'vidlength': vid_length,
                    'viewCountText': vid_view_count_text,
                ...
                #build ._vid_info
                vid._length = vid_metadata['length']
                vid.length_seconds = vid_metadata['length_seconds']
                vid._views = vid_metadata['view_count']
                vid.viewCountText = vid_metadata['viewCountText']
                vid.vidlength = vid_metadata['vidlength']
                # vid.vid_id = vid_metadata['id']
                # vid.vid_url = vid_metadata['url']
                # vid.vid_channel_name = vid_metadata['channel_name']
                # vid.vid_channel_url = vid_metadata['channel_url']
